module.exports = function(grunt) {
    ...
    grunt.initConfig({ // package.json
        pkg: grunt.file.readJSON("package.json"), // typescript tsc
        ts: { options: {}, dev: { src: ["**/*.ts", "!node_modules/**"] } }, // grunt watch
        watch: { scripts: { files: ["**/*.ts", "**/*.html", "**/*.css"], tasks: ["copy"] } }, // files: ['*.js', '!bundle.client.min.js', '!leaderboards.min.js'], //, '!bundle.client.min.js', '!leaderboards.min.js'],
        // tasks: ['browserify'],} },
        // clean
        clean: ["../wingdom-server-dist/*", "!../wingdom-server-dist/.git"], // broserify
        browserify: { "bundle.client.min.js": ["client.js"], "leaderboards.min.js": ["leaderboards.js"] }, // uglify client
        uglify: { // my_target_server:
            // {
            //     options:
            //     {
            //         compress:
            ...
            //     }
            // },
            dist: { options: { compress: { drop_console: true }, mangle: true, beautify: false, output: { comments: false } }, files: { "bundle.client.min.js": ["bundle.client.min.js"], "leaderboards.min.js": ["leaderboards.min.js"] } },
            dev: { options: { compress: false, mangle: false, beautify: true }, files: { "bundle.client.min.js": ["bundle.client.min.js"], "leaderboards.min.js": ["leaderboards.min.js"] } }
        ...
        copy: {
            main: {
                files: [
                    { expand: true, src: '**', cwd: "src/public", dest: 'dist/public/', filter: 'isFile' }
                    // { src: ["./bundle.client.min.js"], dest: "./dist/client/" }, { src: ["./leaderboards.min.js"], dest: "./dist/client/" }, { src: ["./assets/tilemaps/joust-alpha-1.tmx"], dest: "./dist/client/" }
                ]
            }
        }, // server files
        ...
        // { src: ['./primus/primus.js'], dest: '../wingdom-server-dist/' },
        ...
        // aws_s3: { options: { accessKeyId: "<%= aws.AWSAccessKeyId %>", secretAccessKey: "<%= aws.AWSSecretKey %>" }, dist: { options: { bucket: "com.dfeddon.wingdom" }, files: [{ expand: true, cwd: "dist/client", src: ["**"], dest: "./", action: "upload" }] } }
    });
    // dev:
    // {
    //     options:
    ...
    // Default task(s).
    grunt.registerTask('default', ["copy"]); //["ts:dev"]);
    // grunt.registerTask('default', ['browserify']);
    // grunt.registerTask('dist', ['clean', 'browserify', 'uglify:dist', 'copy', 'aws_s3']);
};